...
class CornersProblem(search.SearchProblem):
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                remaining = state[1][:]
                if updated in remaining:
                    remaining.remove(updated)
                successors.append(((updated, remaining), action, 1))
...
def cornersHeuristic(state, problem):
    ...
    while remaining:
        ...
        for corner in remaining:
            distance.append((util.manhattanDistance(current, corner), corner))

        mindistance, corner = min(distance)
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    remainingF = foodGrid.asList()
    distance = []
    if len(remainingF) == 0:
        return 0
    for food in remainingF:
