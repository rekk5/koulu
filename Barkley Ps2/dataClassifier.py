...
def enhancedFeatureExtractorDigit(datum):
    ...
    def inBounds(pos):
        x, y = pos
        return x in range(DIGIT_DATUM_WIDTH) and y in range(DIGIT_DATUM_HEIGHT)

    
    def getNeighbours(pos):
        x, y = pos
        prod = list(product([x - 1, x, x + 1], [y - 1, y, y + 1]))
        prod.remove((x, y))
    ...
    def bfs(x, y, visited):
        ...
        while queue:
            node = queue.pop(0)
            for next in getNeighbours(node): 
                if next not in visited and features[next] == 0: 
                    queue.append(next) 
                    visited.add(next)
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            if features[(x, y)] == 0 and (x, y) not in visited:
                bfs(x, y, visited)
                segments += 1
    
    features["singleSegment"] = int(segments == 1) 
    features["doubleSegment"] = int(segments == 2) 
    features["tripleSegment"] = int(segments == 3) 
...
def enhancedPacmanFeatures(state, action):
    ...
    features["win"] = successorState.isWin()
    features["lose"] = successorState.isLose()
    features["numGhosts"] = len(ghosts)
    features["numFood"] = len(food)


    def closestDistTo(positions):
        return min(map(lambda x: util.manhattanDistance(pacmanPosition, x), positions))

    features["closestGhost"] = closestDistTo(ghosts)
    features["closestFood"] = closestDistTo(food)
